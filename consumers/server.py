...
def run_server():
    ...
    if topic_check.topic_exists("TURNSTILE_SUMMARY") is False:
        ...
        exit(1)
    if topic_check.topic_exists("org.chicago.cta.transformedstations") is False:
        logger.fatal(
            "Ensure that Faust Streaming is running successfully before running the web serve`r!"
        )
        exit(1)
    ...
    consumers = [
        ...
        KafkaConsumer(
            #consumer for FAUST Transformed Stations event 
            "org.chicago.cta.transformedstations",
            lines.process_message,
            offset_earliest=True,
            is_avro=False
        ),
        KafkaConsumer(
            #consumer for Weahther data 
            "org.cta.weather.info",
            weather_model.process_message,
            offset_earliest=True,
            is_avro=True
            ),
        KafkaConsumer(
            #Consumer  for arrival event 
            #"com.cta.stations.arrivals",
            "^com.cta.stations.arrivals.*$",
            lines.process_message,
            offset_earliest=True,
            is_avro=True
        ...
        KafkaConsumer(
            #Consumer for KSQL Turnstile event data 
            "TURNSTILE_SUMMARY",
            lines.process_message,
            offset_earliest=True,
            is_avro=False
